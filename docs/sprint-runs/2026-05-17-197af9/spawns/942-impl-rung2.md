...
1. **pinder-core PR** (branch: `rung-2-sonnet-4-6/942-core`) — the core engine change + invariant guard + reverse-verification test.
2. **pinder-web PR** (branch: `rung-2-sonnet-4-6/942-web`) — the prefetch-fallback fix, which references the new pinder-core sha via submodule bump in the same PR.
...
git fetch origin
git worktree add /tmp/work-rung-2-942-core origin/main
cd /tmp/work-rung-2-942-core
git checkout -b rung-2-sonnet-4-6/942-core
...
5. **Read the anchors:**
   - `/tmp/work-rung-2-942-core/src/Pinder.Core/Conversation/GameSession.cs` — focus on `StartTurnAsync` (line ~791) and any helpers it calls before the outcome-check throw.
   - `/tmp/work-rung-2-942-web/src/Pinder.GameApi/Services/ActiveSession.cs` — focus on `EnsureTurnStartedLockedAsync` (~line 1402), `ResolveTurnAsync` (~1848), and how prefetch faults are caught.
6. `grep -rn "GameEndedException" /tmp/work-rung-2-942-core/src /tmp/work-rung-2-942-web/src` — map every throw site and catch site.
...
1. Build pinder-core baseline: `cd /tmp/work-rung-2-942-core && dotnet build -c Release 2>&1 | tail -10`.
...
9. Open core PR: `gh pr create --repo decay256/pinder-core --base main --head rung-2-sonnet-4-6/942-core --fill`. Body must include `Closes #942` and the `## DoD Evidence` block per spec.
...
1. In `/tmp/work-rung-2-942-web`: `git submodule update --init --remote pinder-core` (or whichever submodule path is used). Verify `git -C pinder-core log -1 --oneline` shows the core merge sha.
...
4. `cd /tmp/work-rung-2-942-web && pnpm install && pnpm test` (or whatever the test runner is — check `package.json` and `AGENTS.md`).

...
git fetch origin
git worktree add /tmp/work-950 origin/main
cd /tmp/work-950
git checkout -b fix/950-stake-not-surfacing
```
...
git push -u origin fix/950-stake-not-surfacing
gh pr create --repo decay256/pinder-core --base main --head fix/950-stake-not-surfacing \
...
Worktree: `/tmp/work-950`.

Per WORKSPACE-ISOLATION: only operate inside `/tmp/work-950/`. Never edit `/root/projects/pinder-core/` directly.

...
**File:** `/tmp/work-942-web/src/Pinder.GameApi/Services/ActiveSession.cs`
...
```bash
cd /tmp/work-942-web/frontend 2>/dev/null && pnpm test 2>&1 | tail -10
```
...
PR:
```bash
gh pr create --repo decay256/pinder-web --base main --head fix/942-prefetch-surface-ghosted \
...
- Do not work in `/root/projects/pinder-web/` — only in `/tmp/work-942-web/`.

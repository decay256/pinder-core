...
git fetch origin
git worktree add /tmp/work-651 origin/main
cd /tmp/work-651
git checkout -b fix/651-replay-availability
...
git push -u origin fix/651-replay-availability
gh pr create --repo decay256/pinder-web --base main --head fix/651-replay-availability \
...
Worktree: `/tmp/work-651`.

Per WORKSPACE-ISOLATION: only operate inside `/tmp/work-651/`. Never edit `/root/projects/pinder-web/` directly.

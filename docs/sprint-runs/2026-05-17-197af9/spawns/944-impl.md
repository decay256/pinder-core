...
git fetch origin
git worktree add /tmp/work-944 origin/main
cd /tmp/work-944
git checkout -b fix/944-trap-activated-wire-field
```
...
6. Find the upstream type that should carry `trap_activated`: `grep -rn "trap_activated\|TrapActivated\|trapActivated" /root/projects/pinder-web/src/ /tmp/work-944/pinder-core/src/ 2>/dev/null | head -20`. The pinder-core `TurnResult` or `TurnRecord` type likely has a `string? TrapActivated` or similar property.
...
One commit. PR via:
```bash
gh pr create --repo decay256/pinder-web --base main --head fix/944-trap-activated-wire-field \
...
- Do not work in `/root/projects/pinder-web/` — only in `/tmp/work-944/`.

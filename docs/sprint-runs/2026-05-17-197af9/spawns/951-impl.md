...
git fetch origin
git worktree add /tmp/work-951 origin/main
cd /tmp/work-951
git checkout -b fix/951-opening-scene-literal
```
...
git push -u origin fix/951-opening-scene-literal
gh pr create --repo decay256/pinder-core --base main --head fix/951-opening-scene-literal \
...
Worktree: `/tmp/work-951`.

Per WORKSPACE-ISOLATION: only operate inside `/tmp/work-951/`. Never edit `/root/projects/pinder-core/` directly.

# Wire Field: `datee_defense_snapshot`
...
| Property | Value |
|---|---|
| JSON key (on `TurnStart`) | `datee_defense_snapshot` (carried as a C# property) |
| C# type | `DateeDefenseSnapshot` |
...
```csharp
// src/Pinder.Core/Conversation/DateeDefenseSnapshot.cs
...
public sealed class DateeDefenseEntry
{
    ...
    public int EffectiveModifier { get; }       // shadow-adjusted + DateeDCIncrease trap bonus

    [JsonPropertyName("base_modifier")]
    public int BaseModifier { get; }            // datee.Stats.GetBase(defendingStat)
}
```
...
```csharp
int effectiveModifier = _datee.Stats.GetEffective(defenderStat);
// GetEffective = GetBase(stat) - floor(GetShadow(ShadowPairs[stat]) / 3)

// Include DateeDCIncrease trap bonus for this attacker stat (if active):
...
Datee has `SelfAwareness=3`, `Overthinking=6`:
...
### Example: DateeDCIncrease trap

Active `Rizz` trap with `DateeDCIncrease +3`, datee `Wit=2`, no shadow:
...
The `DateeDefenseSnapshot` exposes the datee's current defense posture at
the start of each turn so the frontend can:

- Render per-stat success probability without knowing the DC formula.
- Surface the effect of active traps on defense (e.g., `DateeDCIncrease` traps
  making a stat harder to beat).
- Show shadow corruption degrading the datee's defense stats (which makes the
  datee *easier* to beat, reducing `EffectiveModifier` below `BaseModifier`).
- Enable replay/resimulation tooling to reproduce the same defense context
  deterministically from a `TurnSnapshot`.

## Serialization

`DateeDefenseSnapshot` uses `System.Text.Json` with `[JsonPropertyName]`
...
`[JsonPropertyName("datee_defense_snapshot")]` attribute on `TurnStart.DateeDefenseSnapshot`
...
`dateeDefenseSnapshot` as a new optional trailing parameter (defaults `null`
for backward compatibility) and projects `DateeDefenseEntry` values into
`TurnDefenseEntry`.

...
**Related:** #53 (DateeTimingCalculator — similar pattern, datee-side timing)

---

## 1. Overview

In Pinder's asynchronous messaging model, the player's reply speed affects the datee's Interest. If the player takes too long to reply, the datee loses interest — but the penalty is modified by the datee's personality stats. `PlayerResponseDelayEvaluator` is a **pure function** that takes a delay duration, the datee's stat block, and the current interest state, and returns a penalty result containing the interest delta and an optional conversational test trigger. It lives in `Pinder.Core.Conversation` and has zero side effects.
...
- **Input:** delay = 5 minutes, dateeStats = (Chaos base 2, all shadows 0), currentInterest = `Interested`
- **Output:** `DelayPenalty(interestDelta: 0, triggerTest: false, testPrompt: null)`

### Example 2 — 30-minute delay, interest below threshold
- **Input:** delay = 30 minutes, dateeStats = (Chaos base 2, all shadows 0), currentInterest = `Interested` (interest value somewhere in 5–15)
- **Output:** `DelayPenalty(interestDelta: 0, triggerTest: false, testPrompt: null)`
...
- **Input:** delay = 30 minutes, dateeStats = (Chaos base 2, all shadows 0), currentInterest = `VeryIntoIt`
- **Output:** `DelayPenalty(interestDelta: -1, triggerTest: false, testPrompt: null)`

### Example 4 — 3-hour delay, triggers test
- **Input:** delay = 3 hours, dateeStats = (Chaos base 2, all shadows 0), currentInterest = `Interested`
...
- **Input:** delay = 3 hours, dateeStats = (Chaos base 4, all shadows 0), currentInterest = `Interested`
- **Output:** `DelayPenalty(interestDelta: 0, triggerTest: false, testPrompt: null)`
...
- **Input:** delay = 12 hours, dateeStats = (Chaos base 2, Fixation shadow 6), currentInterest = `Interested`
...
- **Input:** delay = 2 hours, dateeStats = (Chaos base 2, Overthinking shadow 6), currentInterest = `Interested`
...
- **Input:** delay = 2 hours, dateeStats = (Chaos base 2, Fixation shadow 7, Overthinking shadow 8), currentInterest = `Interested`
...
- **Input:** delay = 48 hours, dateeStats = (Chaos base 2, all shadows 0), currentInterest = `Bored`
...
If the datee's **base stat** for `StatType.Chaos` (via `dateeStats.GetBase(StatType.Chaos)`) is ≥ 4, the entire penalty is zeroed. The returned `DelayPenalty` has `InterestDelta = 0`, `TriggerTest = false`, `TestPrompt = null`. This check takes priority over all other personality modifiers — if Chaos ≥ 4, no other modifiers are evaluated.

### AC4: Fixation shadow ≥ 6 doubles penalty
If the datee's shadow stat `ShadowStatType.Fixation` (via `dateeStats.GetShadow(ShadowStatType.Fixation)`) is ≥ 6, the base penalty is **doubled** (e.g. −2 becomes −4). This is applied before the Overthinking modifier.

### AC5: Overthinking shadow ≥ 6 applies +1 additional penalty
If the datee's shadow stat `ShadowStatType.Overthinking` (via `dateeStats.GetShadow(ShadowStatType.Overthinking)`) is ≥ 6, the penalty magnitude increases by 1 (e.g. −2 becomes −3, or if Fixation already doubled to −4, becomes −5). Applied after Fixation doubling.
...
### Zero-stat datee
- An datee with all base stats = 0 and all shadow stats = 0: all modifiers are inactive, pure base penalty applies.
...
| `dateeStats` is null | Throw `ArgumentNullException` with parameter name `"dateeStats"` |
...
|---|---|---|
| `StatBlock` | `Pinder.Core.Stats` | Read datee's Chaos base stat and shadow stat values |
...
- **#53 (DateeTimingCalculator):** Related but independent. Computes datee reply delays. `PlayerResponseDelayEvaluator` computes penalties for player reply delays. They do not call each other.
...
3. **Check Chaos override**: if `dateeStats.GetBase(StatType.Chaos) >= 4`, return `DelayPenalty(0, false, null)` immediately.
4. **Apply Fixation doubling**: if `dateeStats.GetShadow(ShadowStatType.Fixation) >= 6`, multiply penalty by 2 (e.g. −2 → −4).
5. **Apply Overthinking addition**: if `dateeStats.GetShadow(ShadowStatType.Overthinking) >= 6`, subtract 1 more (e.g. −4 → −5).
6. **Determine TriggerTest**: set `TriggerTest = true` only if the delay is in the 1–6h bucket AND the final `InterestDelta` is non-zero. Set `TestPrompt` to a descriptive string (e.g. `"Datee noticed the long gap"`) when triggering.
...
The `TestPrompt` string is a hint for the LLM layer. For prototype maturity, the exact string content is not critical — it just needs to be non-null when `TriggerTest` is true. A reasonable default: `"Datee noticed the long gap between replies"`. The LLM adapter will use this to flavor the datee's next message. Future iterations may make this more personality-specific.

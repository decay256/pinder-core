...
| `prompts/emotional-reactions.yaml` | `PromptCatalog.LoadFromDirectory` + `EmotionalReactionPromptCatalog` validation + `EmotionalReactionEventCompiler` + private `PinderLlmAdapter.GenerateEmotionalDirectionAsync` | Internal DATEE emotional reaction direction prompts: seven `InterestState` meanings, four relationship-transition instructions, sixty `StatType` x outcome event meanings, wrapper templates for a private traced input artifact, and the private director LLM prompt with temperature/max-token settings. These entries describe how the delivered player message lands for the recipient; they do not draft the final DATEE reply and are not appended to the current visible DATEE prompt. |

...
| `prompts/emotional-reactions.yaml` | `PromptCatalog.LoadFromDirectory` + `EmotionalReactionPromptCatalog` validation + `EmotionalReactionEventCompiler` + private `PinderLlmAdapter.GenerateEmotionalDirectionAsync` + internal DATEE performance prompt builder | Internal DATEE emotional reaction direction prompts: seven `InterestState` meanings, four relationship-transition instructions, sixty `StatType` x outcome event meanings, wrapper templates for a private traced input artifact, the private director LLM prompt with temperature/max-token settings, and the DATEE performance direction wrapper. These entries describe how the delivered player message lands for the recipient and how the validated director fields should shape the next visible DATEE response without drafting final chat text. |
...
| Private DATEE emotional event input wrapper | `data/prompts/emotional-reactions.yaml` | `emotional-reaction-compiled-wrapper` / `emotional-reaction-character-*` |
| Private DATEE emotional director call prompt | `data/prompts/emotional-reactions.yaml` | `emotional-reaction-director` |
| DATEE emotional performance wrapper | `data/prompts/emotional-reactions.yaml` | `emotional-reaction-performance-direction` |

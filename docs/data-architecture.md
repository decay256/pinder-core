...
| `prompts/emotional-reactions.yaml` | `PromptCatalog.LoadFromDirectory` + `EmotionalReactionPromptCatalog` validation + `EmotionalReactionEventCompiler` | Internal DATEE emotional reaction direction prompts: seven `InterestState` meanings, four relationship-transition instructions, sixty `StatType` x outcome event meanings, and wrapper templates for a private traced input artifact. These entries describe how the delivered player message lands for the recipient; they do not draft the final DATEE reply and are not appended to the current visible DATEE prompt. |

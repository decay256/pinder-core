...
### CLI Interface
```
Usage: dotnet run --project session-runner -- --player <name> --datee <name> [--turns <n>] [--agent <scoring|llm|human>] [--model <id>] [--overlay-model <id>] [--difficulty <n>] [--seed <n>] [--debug]
       dotnet run --project session-runner -- --player-def <path> --datee-def <path> [--turns <n>] [--agent <scoring|llm|human>]
  ...
  --turns <n>            Maximum turns (default: 30)
  --agent <type>         Player agent: scoring, llm, or human (default: scoring)
  --model <id>           Specify model ID
  --overlay-model <id>   Specify overlay model ID
  --difficulty <n>       Difficulty setting
  --seed <n>             Random seed
...
- **CLI-driven configuration**: The session runner uses positional CLI arguments instead of environment variables for character selection and agent type. The `--turns` default is 30 (previously hardcoded as 15).
...
| 2026-04-04 | #414 | Added `CharacterLoader` to load characters from prompt files. Replaced all hardcoded stat blocks in `Program.cs` with `CharacterLoader.Load()` calls. Added CLI argument parsing (`--player`, `--datee`, `--turns` is 30, `--agent` replacing `PLAYER_AGENT` env var). Usage/error messages printed to stderr with exit code 1. Added `CharacterLoaderSpecTests.cs` with comprehensive parsing, error, and edge-case coverage. |
...
| 2026-04-05 | #514 | Fixed hardcoded "Sable"/"Brick" names in DC Reference table header and column header in `Program.cs`. Now uses `player1`/`player2` string interpolation for dynamic character names. Added `DcTableHeaderTests.cs` (4 tests) verifying interpolation with custom names, standard names, and source file assertion that no hardcoded header remains. |
| 2026-04-05 | #516 | Fixed `CharacterLoader.ParseLevel` bug: corrected stale level values in character JSON files (velvet: 4→7, brick: 6→9, zyx: 4→1). Added `ParseLevel` tests to `CharacterLoaderTests.cs` (standard format, single/double digit, missing line fallback, parameterized starter characters). Made `ParseLevel` visible as `internal static`. |

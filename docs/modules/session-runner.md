...
| 2026-04-06 | #573 | Removed the LLM call for Narrative Beats. Program.cs no longer prints the ✨ emoji prefix for narrative beats, as they are now hardcoded status strings. |
| 2026-04-06 | #574 | Bug fix — CharacterDefinitionLoader.Parse now passes the bio field to the CharacterProfile constructor, ensuring bios are correctly populated when loading from JSON definitions. |

...
- **`session-runner/PlaytestFormatter.cs`** — Static utility class for formatting player agent reasoning blocks, option score tables, and combo explanations as markdown. Contains `FormatReasoningBlock`, `FormatScoreTable`, `GetComboSequenceDescription`, and `GetComboRewardSummary`.
...
| 2026-04-06 | #526 | Added combo sequence explanations to playtest output via `PlaytestFormatter.GetComboSequenceDescription` and `GetComboRewardSummary`. Explanations now appear under dialogue options with pending combos and when a combo successfully triggers. Added `Issue526_ComboExplanationTests.cs` for coverage. |

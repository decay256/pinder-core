...
- **`SuccessDeliveryInstruction`** (`const string`) — §3.3: Delivers the player's intended message on a successful roll. Defines three margin-based tiers aligned with `SuccessScale`: Clean (1–4) delivers essentially as written, Strong (5–9) sharpens phrasing/timing/rhythm without adding new ideas, Critical/Nat 20 (10+) delivers at peak execution. Contains an explicit counterpart rule: every idea in the delivered version must have a counterpart in the intended version. Additions sharpen, not expand. Retains `{player_name}` placeholder for runtime substitution by `SessionDocumentBuilder.BuildDeliveryPrompt()`.

...
| 2026-08-27 | #1426 | DATEE performance output now uses the provider-neutral `datee_performance.v1` structured contract. The adapter requires structured transport for DATEE performance, validates the complete JSON object strictly, rejects legacy signal markers in visible messages, records only `message` into semantic history, and keeps tell/weakness data in trusted result metadata. |

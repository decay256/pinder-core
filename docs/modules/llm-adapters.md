...
| `Anthropic/AnthropicLlmAdapter.cs` | (Historical) Adapter implementing the LLM interface using Anthropic's API. Replaced by `PinderLlmAdapter` + `ILlmTransport`. |
...
| `src/Pinder.Core/Interfaces/IStatefulLlmAdapter.cs` | (Historical) Interface for stateful mode. We now use `GameSession` for history-passing to `PinderLlmAdapter` instead of `StartConversation` or `HasActiveConversation`. |
...
| `AnthropicLlmAdapterStatefulTests.cs` (Historical) (test) | (Historical) 12 tests for stateful `AnthropicLlmAdapter` behavior across all 4 `ILlmAdapter` methods |
...
| `Issue542_StatefulSession_TestEngineerTests.cs` (test) | (Historical) Spec-driven tests for deprecated `IStatefulLlmAdapter` interface shape. |
...
### `IStatefulLlmAdapter` (Historical)

This interface was removed. `GameSession` now holds all engine-owned history and uses history-passing to interact with the stateless `PinderLlmAdapter`. The legacy `StartConversation` and `HasActiveConversation` methods have been deleted. `AnthropicLlmAdapter` and `OpenAiLlmAdapter` are also removed.

### `ConversationSession` (public sealed class)

Accumulates user/assistant messages for multi-turn conversations with `ILlmTransport` implementations. System blocks are set once at construction; messages grow unbounded as turns are played.
...
### `AnthropicLlmAdapter` (Historical)

Deprecated. We now use `PinderLlmAdapter` with history-passing.
...
| 2026-04-05 | #541 | Stateful conversation mode — Added `ConversationSession` class (`Pinder.LlmAdapters`) that accumulates user/assistant `Message` objects and builds `MessagesRequest` with cached system blocks (ephemeral `CacheControl`) + full message history snapshot. Historical `AnthropicLlmAdapter` gains deprecated `StartConversation(string systemPrompt)` and `HasActiveConversation` property. When active, all active `ILlmAdapter` methods append to and read from the session instead of building fresh single-message requests. Stateless fallback is preserved when no session is active. `ILlmAdapter` interface unchanged. Tests: `ConversationSessionTests.cs` (16), `AnthropicLlmAdapterStatefulTests.cs` (Historical) (12), `Issue541_StatefulConversationTests.cs`, `Issue541_AdditionalTests.cs`. |
| 2026-04-05 | #542 | `IStatefulLlmAdapter` interface + GameSession wiring — Deprecated `IStatefulLlmAdapter` interface formalizes historical `StartConversation(string)` and `HasActiveConversation` as a sub-interface of `ILlmAdapter`. `AnthropicLlmAdapter` (historical) class declaration changed from `ILlmAdapter` to `IStatefulLlmAdapter`. `GameSession` 6-parameter constructor historically checked `IStatefulLlmAdapter` and called `StartConversation`. `NullLlmAdapter` unchanged — stateless path preserved. Tests: `Issue542_StatefulSession_TestEngineerTests.cs`. |
...
| 2026-04-06 | #572 | Bug fix — Removed the `[RESPONSE]` tag requirement from `PromptTemplates.DateeResponseInstruction` so the LLM outputs message text directly. Updated Historical `AnthropicLlmAdapter.ParseDateeResponse` to extract text before `[SIGNALS]` and gracefully strip legacy `[RESPONSE]` tags or quotes if generated. |
...
| 2026-04-06 | #534 | Added `--debug` flag support to `session-runner` via `AnthropicOptions.DebugDirectory`. Historical `AnthropicLlmAdapter` now intercepts and writes `turn-{turn:D2}-{callType}-request.json` and `response.json` for every API call, plus a `session-summary.json` tracking cumulative input/output and cache tokens. Validated thread-safe stat tracking with 100-thread concurrent test. |

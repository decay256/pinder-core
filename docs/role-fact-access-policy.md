...
Durable rejection journaling requires the caller's real request ID; missing
correlation fails before persistence and is never replaced with a synthetic value.
Host-sink persistence is fail-closed: a failed write emits a terminal failed
diagnostic and propagates `AgentJournalSinkPersistenceException` before provider,
dice, history, or state mutation.

...
namespace Pinder.Core.Tests
{
    ...
    public class Issue833_BulletListPromptSectionsTests
    {
        ...
        [Fact]
        public void BackstorySection_EmitsBulletList_AndDoesNotDumpLieRealityTable()
        {
            var fragments = BuildFragments(
            ...
            Assert.DoesNotContain("LIE:", prompt);
            Assert.DoesNotContain("REALITY:", prompt);
        }

        [Fact]
        public void PsychiatricDiagnosis_IsNotRenderedInSystemPrompt()
        {
            ...
            Assert.DoesNotContain("THERAPIST DIAGNOSIS", prompt);
        }
        ...
        private static string ExtractSection(string prompt, string header, string nextHeader)
        {
            ...
            int start = prompt.IndexOf(header, searchFrom, System.StringComparison.Ordinal);

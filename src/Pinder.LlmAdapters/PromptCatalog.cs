...
private static readonly Regex PlaceholderToken = new Regex(@"\{[A-Za-z_][A-Za-z0-9_]*\}", RegexOptions.CultureInvariant);
private const string StyleDirective = @"\b(?:always|must|will|never|only|use|uses|include|includes|follow|follows|are|is|has|have|end|ends)\b[^.\n]{0,96}";
private const string MessageSubject = @"\b(?:replies?|messages?|texts?)\b";
private static readonly Regex[] PersonalitySurfaceStylePatterns =
{
StylePattern(MessageSubject + @"[^.\n]{0,48}" + StyleDirective + @"\b(?:end|finish|punctuation|ellipsis|periods?|question marks?|exclamation marks?)\b"),
StylePattern(MessageSubject + @"[^.\n]{0,48}" + StyleDirective + @"\b(?:lowercase|uppercase|capitalization|casing)\b"),
StylePattern(StyleDirective + @"\b(?:line breaks?|new lines?|paragraph breaks?|emoji)\b"),
StylePattern(MessageSubject + @"[^.\n]{0,64}\b(?:open|start|begin)s?\b"),
StylePattern(MessageSubject + @"[^.\n]{0,64}" + StyleDirective + @"\b(?:sentence|template|format|cadence|clipped|short)\b"),
};
private static Regex StylePattern(string pattern) => new Regex(pattern, RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);
...
=> RuntimeSystemPromptKeys.Concat(RuntimeCompletePromptKeys).Concat(EmotionalReactionPromptCatalog.RuntimeActiveKeys).Distinct(StringComparer.Ordinal).ToArray();
...
throw new PromptLayerContractException("prompt_contract.placeholder.unresolved", null, PromptContractRoleScope.RoleNeutral, contract.Key, field, entry.SourceFile, SourceSpan(entry), null);
}
}
}
...
{
var personality = RequireCompleteEntry("personality_consolidation", "prompt-catalog: missing required key 'personality_consolidation'.");
string source = (personality.SystemPrompt ?? string.Empty) + "\n" + (personality.UserTemplate ?? string.Empty);
string[] prohibited = { "TEXTING STYLE SIGNALS", "{texting_style}", "texts under pressure" };
if (prohibited.Any(phrase => source.IndexOf(phrase, StringComparison.OrdinalIgnoreCase) >= 0)
|| PersonalitySurfaceStylePatterns.Any(pattern => pattern.IsMatch(source)))
throw PersonalitySurfaceStyleFailure(personality);
foreach (PromptEntry entry in _entries.Values)
if (((entry.SystemPrompt ?? string.Empty) + "\n" + (entry.UserTemplate ?? string.Empty)).IndexOf("DATEE is another PLAYER's character", StringComparison.OrdinalIgnoreCase) >= 0)
throw new PromptLayerContractException("prompt_contract.legacy_datee_ownership", null, PromptContractRoleScope.RoleNeutral, "active_catalog", "configured_text", entry.SourceFile, SourceSpan(entry), null);
}
private static PromptLayerContractException PersonalitySurfaceStyleFailure(PromptEntry entry)
=> new PromptLayerContractException("prompt_contract.personality.surface_style", "synthesis", PromptContractRoleScope.RoleNeutral, "personality_consolidation", "configured_text", entry.SourceFile, SourceSpan(entry), null);
private static string? SourceSpan(PromptEntry entry) => entry.SourceLine.HasValue ? "line:" + entry.SourceLine.Value : null;
...
public PromptEntry(string? systemPrompt, string? userTemplate, string? sourceFile = null, double? temperature = null, int? maxTokens = null, int? sourceLine = null)
{
SystemPrompt = systemPrompt;
UserTemplate = userTemplate;

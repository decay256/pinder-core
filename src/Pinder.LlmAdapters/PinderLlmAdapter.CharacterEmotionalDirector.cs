                    ...
                    try
                    {
                        ...
                        if (canUseStructured)
                        {
                            ...
                            var response = await SendStructuredWithDiagnosticsAsync(
                                    ...
                                    promptContract: new PromptProviderContract(invocation.RecipientRole, new[] { systemDocument, userDocument }, invocation.RoleFactAccessDecisions, request.SchemaName + ":" + request.SchemaVersion))
                                .ConfigureAwait(false);
                        ...
                        else
                        {
                            responseText = await SendWithDiagnosticsAsync(
                                    ...
                                    invocation.PriorMessages,
                                    callId: journal.CallId,
                                    promptContract: new PromptProviderContract(invocation.RecipientRole, new[] { systemDocument, userDocument }, invocation.RoleFactAccessDecisions))
                                .ConfigureAwait(false);
                            direction = ParseCharacterDirectionOrThrow(
                                responseText,
                                false,

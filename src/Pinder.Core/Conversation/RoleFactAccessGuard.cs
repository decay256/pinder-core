...
namespace Pinder.Core.Conversation
{
    internal static class RoleFactAccessGuard
    {
            ...
            Action<OperationalDiagnosticEvent>? onDiagnostic)
        {
            ...
            {
                var correlationError = new RoleFactContractException(
                    "agent_journal.request_id.required",
                    "Durable role-fact rejection journaling requires a real request ID.");
                OperationalDiagnostics.Emit(
                    onDiagnostic,
                    AgentJournalOperationalDiagnostics.RoleFactPolicyCorrelationRejected(
                        correlationError,
                        decision,
                        context,
                        operationKind,
            ...
            {
                var persistenceError = sinkError as AgentJournalSinkPersistenceException
                    ?? new AgentJournalSinkPersistenceException(
                        sinkRecord.RecordId,
                        sinkRecord.CustomType,
                        sinkError);
                OperationalDiagnostics.Emit(
                    onDiagnostic,
                    AgentJournalOperationalDiagnostics.SinkPersistenceFailed(
                        ...
                        persistenceError.InnerException ?? persistenceError));

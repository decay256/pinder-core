...
namespace Pinder.RemoteAssets
{
    ...
    public sealed class SyncHelper
    {
        ...
        public SyncHelper(Configuration config, HttpClient http, ILogger<SyncHelper> logger)
        {
            ...
            CancellationToken ct = default)
        {
            ...
            var op = RemoteAssetOperationLog.Begin(
                _logger,
                "remote_asset.publish",
                assetId: metadata.CharacterId,
                    ...
                    })
                    {
                        ...
                        try
                        {
                            ...
                            if (status == 200 || status == 201)
                            {
                                ...
                                CharacterAssetMetadata result = ParsePublishResponse(body);
                                op.Complete(
                                    "success",
        ...
        public async Task SaveAsync(CharacterDefinition def, CancellationToken ct = default)
        {
            ...
            try
            {
                await PublishAsync(def, metadata, ct).ConfigureAwait(false);
                op.Complete("success");
            }
            catch (Exception ex)
            {
        ...
        {
            var op = RemoteAssetOperationLog.Begin(_logger, "remote_asset.delete", assetId: characterId);
            try
            {
                    ...
                    using (var req = new HttpRequestMessage(HttpMethod.Delete, EigencoreCharacterStoreRead.BuildAssetUri(characterId)))
                    {
                        ...
                        try
                        {
                            ...
                            if (status == 200 || status == 204)
                            {
                                ...
                                op.Complete("success", statusCode: status);
                                return true;
                            }
                            ...
                            if (status == 404)
                            {
                                if (resp.Content != null)
                                    await resp.Content.ReadAsByteArrayAsync().ConfigureAwait(false);
                                op.Complete("not_found", statusCode: status);
                                return false;
                            }
